...
Page({
  ...
  subinp_Fn:function(e){
    ...
    var that = this
    var url = app.globalData.host + '/buildResources/saveReview';
    var data = {
      // id: options.id,
      communityId:that.data.listdelit.id,
  ...
  lianxiFn: function (e) {
    ...
    wx.navigateTo({
      url: '/pages/lianxijingjiren/index?commId=' +this.data.listdelit.id,
    })
  },
  zu_Fn:function(e){
    wx.navigateTo({
  ...
  getMore: function (page) {
    wx.showLoading({
      title: '加载中',
		...
		var that = this;
    var url = app.globalData.host + '/buildResources/getReviewList';
    var data = {
      communityId:that.data.listdelit.id,
      page:page,
		}
  ...
  onLoad: function (options) {
    ...
    var url = app.globalData.host + '/community/getBlsqHouseByComSel';
    var data = {
      esId: options.id
    }
    app.wxRequest('POST', url, data, (res) => {
      ...
      wx.hideLoading()
      if (res.success) {
        for(var index in res.data.lecList){
          res.data.lecList[index].bedroom_number = parseInt( res.data.lecList[index].bedroom_number)
          res.data.lecList[index].floor = parseInt( res.data.lecList[index].floor)
          res.data.lecList[index].house_type = parseInt( res.data.lecList[index].house_type)
        }
        for(var index in res.data.selList){
          res.data.selList[index].bedroom_number = parseInt( res.data.selList[index].bedroom_number)
          res.data.selList[index].floor = parseInt( res.data.selList[index].floor)
          res.data.selList[index].house_type = parseInt( res.data.selList[index].house_type)
        }
